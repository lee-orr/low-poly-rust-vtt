...
use naia_bevy_server::{
    ...
    Plugin as NaiaPlugin, RoomKey, Server, ServerAddrs, ServerConfig, Stage as NaiaStage, UserKey,
};

use crate::shared_lib::{
    protocol::{channels::Channels, Protocol},
    shared_config,
};
...
fn init(mut server: Server<Protocol, Channels>, server_settings: Res<ServerSettings>) {
    ...
    if let (Ok(main_addr), Ok(webrtc_addr)) = (
        format!("0.0.0.0:{}", server_settings.main_port).parse::<SocketAddr>(),
        format!("0.0.0.0:{}", server_settings.webrtc_port).parse::<SocketAddr>(),
    ) {
        let server_addresses =
            ServerAddrs::new(main_addr, webrtc_addr, &server_settings.webrtc_url);

        info!(
            "listening on {}, webrtc on {} - public {}",
            main_addr, webrtc_addr, &server_settings.webrtc_url
        );
...
fn disconnection(mut event_reader: EventReader<DisconnectionEvent>, mut games: ResMut<Games>) {
    for event in event_reader.iter() {
